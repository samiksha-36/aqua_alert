...
export async function sendAlertEmail({ alert, recipients }) {
      ...
      <div style="padding: 24px; background: #ffffff;">
        ...
        <div style="background: #f8fafc; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 6px 0; color: #64748b;">Alert ID</td>
              <td style="padding: 6px 0; font-weight: bold; color: #1e293b;">${alert.alertId}</td>
            </tr>
            <tr>
              <td style="padding: 6px 0; color: #64748b;">Confidence Score</td>
              <td style="padding: 6px 0; font-weight: bold; color: ${color};">${confidence}% (${alert.confidenceLabel})</td>
            </tr>
            <tr>
              <td style="padding: 6px 0; color: #64748b;">District</td>
              <td style="padding: 6px 0; font-weight: bold; color: #1e293b;">${alert.district}, ${alert.state}</td>
            </tr>
            <tr>
              <td style="padding: 6px 0; color: #64748b;">Issued At</td>
              <td style="padding: 6px 0; color: #1e293b;">${new Date(alert.createdAt).toLocaleString('en-IN')}</td>
            </tr>
          </table>
        </div>
        ...
        <div style="background: #f8fafc; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
            <div style="background: white; padding: 10px; border-radius: 6px; text-align: center; border: 1px solid #e2e8f0;">
              ...
              <div style="font-weight: bold; color: #1e293b;">${alert.triggerData.rainfall_mm ?? 'N/A'} mm</div>
              <div style="font-size: 12px; color: #64748b;">Rainfall</div>
            </div>
            <div style="background: white; padding: 10px; border-radius: 6px; text-align: center; border: 1px solid #e2e8f0;">
              <div style="font-size: 20px;">🌊</div>
              <div style="font-weight: bold; color: #1e293b;">${alert.triggerData.river_level_m ?? 'N/A'} m</div>
              <div style="font-size: 12px; color: #64748b;">River Level</div>
            </div>
          </div>
        </div>
        ` : ''}
        ...
        <div style="background: #fef2f2; border-radius: 8px; padding: 16px;">
          <p style="margin: 0; color: #991b1b; font-size: 13px;">
            ...
            NadiBot community reporters in your area have been pinged for ground-truth verification.
            You will receive a follow-up email when confidence score is updated.
          </p>
        </div>
  ...
  await transporter.sendMail({
    from: `"AquaAlert System" <${process.env.GMAIL_USER}>`,
    to: recipients.join(', '),
    ...
    html: `
      <div style="font-family: Arial; max-width: 600px; margin: 0 auto; border: 3px solid #7c3aed; border-radius: 12px; overflow: hidden;">
        <div style="background: #7c3aed; padding: 20px; text-align: center;">
          ...
          <p style="color: #e9d5ff; margin: 8px 0 0;">${alert.district}, ${alert.state} · ${confirmed}/${total} community reporters confirmed</p>
        </div>
        <div style="padding: 24px;">
          <p style="font-size: 18px; color: #1e293b;">Confidence score: <strong style="color: #7c3aed;">${confidence}%</strong></p>
          <p style="color: #475569;">${confirmed} out of ${total} community reporters on the ground have confirmed the ${alert.alertType.toLowerCase()} situation in ${alert.district}.</p>
